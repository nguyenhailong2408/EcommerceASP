var CategoryManage = function () {
    $("#modal-update").off('hidden.bs.modal').on('hidden.bs.modal', function (e) {
        //Common.CategoryManage.SubmitForm();
    });
    $("#modal-update").off('shown.bs.modal').on('shown.bs.modal', function (e) {
        //Common.CategoryManage.ShowDialog();
    });
    return this.Init();
};
...
CategoryManage.prototype = {
    ...
    RegisterEvent: function () {
        this.IsPaging = false;
        var that = this;
        var form = $("#form-search-CategoryManage");
        $("#table-list-CategoryManage > tbody > tr i.fa-edit").unbind("click").click(function () {
            var tr = $(this).closest("tr");
            Common.CategoryManage.ShowDialog(tr.data("id"));
        });
        $("#table-list-CategoryManage > tbody > tr i.fa-trash").unbind("click").click(function () {
            var tr = $(this).closest("tr");
        ...
        $("#btn-update").unbind("click").click(function (e) {
            Common.CategoryManage.SubmitFormUpdate(e);
        });
    },
    SubmitForm: function () {
        $("#form-search-CategoryManage").submit();
    },
    SetPage: function (page) {
        $("#form-search-CategoryManage").find("input[name='PageCurrent']").val(page);
    },
    Paging: function (page) {
        Common.CategoryManage.SetPage(page);
        Common.CategoryManage.IsPaging = true;
        Common.CategoryManage.SubmitForm();
    },
    SuccessForm: function () {
        Common.ShowLoading(false);
        Common.CategoryManage.RegisterEvent();
    },
    BeforeSend: function () {
        Common.ShowLoading(true);
        if (Common.CategoryManage.IsPaging) {
            Common.CategoryManage.SetPage(1);
        }
    },
    ...
    UpdateSuccess: function (res) {
        ...
        if (res.Status) {
            Common.CategoryManage.SubmitForm();
            Common.CategoryManage.HideDialog();
            alert(res.Message);
    ...
    ShowDialog: function (id) {
        Common.Ajax({
            type: "POST",
            url: CategoryManage.Url.FormUpdate,
            cache: false,
            dataType: "html",
            data: { id: id }
        }, function (data) {
    ...
    OnBlurInputSlug: function (e) {
        Common.CategoryManage.GetPageBySlug($(e).val())
    ...
    GetPageBySlug: function (slug) {
        return new Promise(function (resolve, reject) {
            ...
            Common.Ajax({
                type: "POST",
                url: CategoryManage.Url.GetPageBySlug,
                cache: false,
                dataType: "json",
                data: {
                    strSlug: slug
